...
class FeatureContext extends RawDKANContext {
  ...
  public function iClickTheLinkNextToTitle($link, $title) {
    $items = $this->getSession()->getPage()->findAll('xpath', "//span[contains(@class,'views-dkan-workflow-tree-title')]/a[text()='" . "[Feedback] " . $title . "' or text()='" . "[Dataset] " . $title . "']/../../span[contains(@class, 'views-dkan-workflow-tree-action')]/a[text()='" . $link . "']");
    if (empty($items)) {
